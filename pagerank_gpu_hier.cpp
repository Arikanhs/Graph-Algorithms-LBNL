...
int main(int argc, char* argv[]) {
    ...
    typedef Kokkos::View<int*, Kokkos::LayoutLeft, MemSpace>   ViewVectorType;
    typedef Kokkos::View<float*, Kokkos::LayoutLeft, MemSpace>   ViewVectorTypeFloat;
    ...
    ViewVectorType::HostMirror h_out_degree = Kokkos::create_mirror_view( out_degree );

    for(int a = 0; a < vertices; a++){
        h_out_degree(a) = adj[a].size();
    }

    Kokkos::deep_copy(out_degree, h_out_degree);
    ...
    **/
    ...
    ViewVectorType in_neigh_indices("indices pointer view of in_neigh list", edges*2);

    ViewVectorType::HostMirror h_in_neigh_indexPointer = Kokkos::create_mirror_view( in_neigh_indexPointer );
    ViewVectorType::HostMirror h_in_neigh_indices = Kokkos::create_mirror_view( in_neigh_indices );
    ...
    for(int a = 0; a < vertices; a++){
        ...
        //indexPointer[a+1] = i;
        h_in_neigh_indexPointer(a+1) = i;

        // Populate the indices vector
        for(int b = 0; b < size; b++){
            h_in_neigh_indices(count) = in_neigh[a][b];
            count++;
        }
    }

    Kokkos::deep_copy(in_neigh_indexPointer, h_in_neigh_indexPointer);
    Kokkos::deep_copy(in_neigh_indices, h_in_neigh_indices);

    /**
    cout << "lists of in_neigh by each element" << endl;
    for(int u = 0 ; u < vertices; u++){

    	int v = h_in_neigh_indexPointer(u);
	    int limit = h_in_neigh_indexPointer(u+1);
	
	cout << "element " << u  << " : " ;

	for(; v < limit; v++){
	   int v_element = h_in_neigh_indices(v);
	   cout << v_element << " " ;
	}
	cout << " " << endl;
    }
    ...
    **/
    ...
    ViewVectorTypeFloat::HostMirror h_scores = Kokkos::create_mirror_view( scores );

    // start each value with init_score
    for(int i = 0 ; i < vertices; i++){
        h_scores(i) = init_score;
    }

    Kokkos::deep_copy(scores, h_scores);


    //vector<float> outgoing_contrib(vertices);
    ViewVectorTypeFloat outgoing_contrib("outgoing_contrib view", vertices);
    //ViewVectorType::HostMirror h_outgoing_contrib = Kokkos::create_mirror_view( outgoing_contrib );
    ...
    for (int iter=0; iter < max_iters; iter++) {
        ...
        Kokkos::parallel_reduce("Outer Reduction", team_policy(vertices, Kokkos::AUTO), KOKKOS_LAMBDA ( const member_type& team, double &reduction_error) {

            int u = team.league_rank();
	    ...
	    int v = in_neigh_indexPointer(u);
            ...
            Kokkos::parallel_reduce(Kokkos::TeamThreadRange(team, v, limit), [=] (int y, float& incoming_total_reduction){
            //for (; v < limit; v++){
		int v_element = in_neigh_indices(y);    
            	incoming_total_reduction += outgoing_contrib(v_element);
            }, incoming_total);
    ...
    Kokkos::deep_copy( h_scores , scores );

    /**
    for(int a = 0; a < vertices; a++){
    	cout << "score of " << a << " : " << h_scores(a) << endl;  
    }
